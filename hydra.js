...
function halton(startIndex, base) {
  let fraction = 1;
  let result = 0;
...
class Game {
  constructor() {
    ...
    const subWeaponIdx = getRandomInt(subtractingWeapons.length);
    const subWeapon = subtractingWeapons[subWeaponIdx];

    // Make sure gcd of + and - weapons is 1, to guarantee that
    // puzzle is solveable.
    let addWeaponIdx = 0;
    let addWeapon = addingWeapons[addWeaponIdx];
    let count = 100;
    do {
      addWeaponIdx = getRandomInt(addingWeapons.length);
      addWeapon = addingWeapons[addWeaponIdx];
      count -= 1;
    } while (count > 0 && gcd(subWeapon.value, addWeapon.value) !== 1);

    const divWeaponIdx = getRandomInt(dividingWeapons.length);
    const divWeapon = dividingWeapons[divWeaponIdx];

    this.weapons = [subWeapon, addWeapon, divWeapon];
    this.hydra_color = `#${(Math.floor(Math.random() * 0xFFFFFF)).toString(16)}`;
...
function canUse(weapon, numHeads) {
  switch (weapon.op) {
    case '+': return true;
    case '-': return numHeads >= parseInt(weapon.value, 10);
    case '/': return numHeads % parseInt(weapon.value, 10) === 0;
...
function getNewNumHeads(weapon, currNumHeads) {
  let heads = currNumHeads;
  switch (weapon.op) {
...
function search(numHeads, weapons) {
  const startNode = {
    heads: numHeads, score: 0, weapon: '', parent: -1,
  };
  const visited = new Map(); // visited nodes; {num heads} -> {Node}
  const queue = []; // nodes to visit

  queue.push(startNode);

  while (queue.length > 0) {
    const currNode = queue.shift();
    if (visited.has(currNode.heads)) {
      continue;
    }
    visited.set(currNode.heads, currNode);
    if (currNode.heads === 0) {
      break;
    }

    for (const w of weapons) {
      if (!canUse(w, currNode.heads)) {
        continue;
      }
      const newHeads = getNewNumHeads(w, currNode.heads);

      const newNode = {
        heads: newHeads,
        score: +currNode.score + +1,
        weapon: w.name,
        parent: currNode.heads,
      };
      if (!visited.has(newNode.heads)) {
        queue.push(newNode);
      }
    }
  }
...
function solve(numHeads, weapons) {
  const visited = search(numHeads, weapons);

  let currNode = visited.get(0);
  const weaponList = [];
  while (currNode.parent !== -1) {
    weaponList.push(currNode.weapon);
    currNode = visited.get(currNode.parent);
  }
  weaponList.reverse();
  return weaponList;
}
...
function getHead(svg, color, headx, heady, bodyx, bodyy) {
  const dx = headx - bodyx;
  const dy = heady - bodyy;
  const r = Math.sqrt(dx * dx + dy * dy);
  const necklength = r;

  const head = getrawhead(color, necklength);
  const origx = head.center[0];
  const origy = head.center[1];
  const headelem = head.element;
...
function drawHydra(svg, color, numheads) {
  ...
  const hydraGroup = document.createElementNS(svgns, 'g');
  svg.appendChild(hydraGroup);

  const body = getbody(color);
  const bodyelement = body.element;
  const bodyx = body.center[0];
  const bodyy = body.center[1];
  hydraGroup.appendChild(bodyelement);

  // determine head locations
  const seed = getRandomInt(1000);
  const generator = getQuasirandom2DPoint(seed);
  const headlocs = [];
  ...
  for (let i = 0; i < numheads; i += 1) {
    ...
    const headElement = getHead(svg, color, headx, heady, bodyxoffset, bodyyoffset);
    hydraGroup.appendChild(headElement);
  }

  // Shift everything up a little bit.
  const t1 = svg.createSVGTransform();
  t1.setTranslate(0, -20);
  const transformList = hydraGroup.transform.baseVal;
  transformList.appendItem(t1);
}
